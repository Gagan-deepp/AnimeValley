...
export const checkBookMark = async ({ userID, animeID }) => {
    try {
        connectDB();
        let bookMarkerd = false
        const Userbookmarks = await userModel.findOne({ id: userID }, { talks: 0, communities: 0 }).populate({
            path: "bookmarks",
            model: "BookMarks"
        })

        Userbookmarks.bookmarks.forEach(bookMark => {
            if (bookMark.animeID == animeID) {
                bookMarkerd = true
            }
        });
        return bookMarkerd
        ...
        console.log(error)
        throw new Error(`Error while Checking BookMark --> : ${error.message}`)
    }
}
