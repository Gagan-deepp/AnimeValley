...
export async function addMemberToCommunity(organizationID, userID) {
    try {
        ...
        const community = await communityModel.findOne({ id: organizationID })
...
export async function removeMemberFromCommunity(communityID, userID) {
    try {
        connectDB()
        const community = await communityModel.findOne({ id: communityID }, { _id: 1 })
...
export async function updateCommunity(communityID, name, username, image) {
    try {
        connectDB();
        const updatedCommuntiy = await communityModel.findOneAndUpdate({ id: communityID }, { name, userName: username, image })
...
export async function deleteCommunity(communityID) {
    ...
    try {
        ...
        const deletedCommunity = await communityModel.findOneAndDelete({ id: communityID })
        ...
        console.log("deletedCommunityIDDDD : ", deletedCommunity._id)
        ...
        const communityTalks = await talkModel.find({ communities: deletedCommunity._id });
        const talkPromise = communityTalks.map((talk) => {
            talk.communities.pull(communityID);
            return talk.save();
        });

        const allpromises = [updateUserPromises, talkPromise]
        ...
        console.log(error)
        throw new Error(`Error While Deleting Community: ${error.message}`)
    }
}