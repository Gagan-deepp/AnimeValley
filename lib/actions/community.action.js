...
export async function fetchCommunityInfo(id) {
    try {
        connectDB();
        const communityData = await communityModel.findOne({ uniqueID: id })
            .populate({ path: "members", model: "User" })
            .populate({ path: "talks", model: "Talks" })
        return communityData;
    } catch (error) {
        console.log(error)
        throw new Error(`Error while fetching Community Info : ${error.message}`)
    }
}

export async function commmunityTalks(id) {
    try {
        connectDB();
        ...
        console.log(error)
        throw new Error("Error While Fetching Talk from Community : ", error.message)
    }
}
