...
export const searchAll = async (query, type) => {
    ...
    if (type) {
        response = await fetch(`https://api.jikan.moe/v4/anime?q=${query}&limit=15&sfw=true&type=${type}`);
    } else {
        response = await fetch(`https://api.jikan.moe/v4/anime?q=${query}&limit=15&sfw=true`);
    }
